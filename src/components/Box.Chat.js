                ...
                <AiOutlineCloseSquare
                  ...
                  onClick={() => {
                    if (state.isNewMessage) {
                      ...
                      setOneState("allMessages", state.sourceMessage);
                    } else {
                      // @ts-ignore
                      dispatch({
                        type: IS_NEW_MESSAGE,
          ...
          {state.isNewMessage ? (
            ...
            <div className="flex rounded-2xl md:rounded-full w-full border-[1px] border-[#8eabb4] px-1 md:px-2 my-2 ">
              <input
                ...
                onChange={(e) => {
                  setOneState("textSearchPeople", e.target.value);
                  if (!e.target.value) {
                    getData();
                  ...
                  const arrFilter = state.sourceMessage.filter((v) => {
                    const length = v.members.length;
                    for (var i = 0; i < length; i++) {
                      if (v.members[i]._id === user._id) {
                        continue;
                      }
                      if (
                        v.members[i].name
